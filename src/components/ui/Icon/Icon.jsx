...
export default function Icon({ image, text, link, style }) {
  return (
    <Link to={link} className="w-16 flex flex-col items-center gap-2 group">
      ...
      <p className={`text-[#f1f1f1] text-sm font-light text-center transition-colors duration-200 group-hover:bg-blue-900 group-hover:text-white px-1 ${style}`}>{text}</p>
    </Link>
  )
}
